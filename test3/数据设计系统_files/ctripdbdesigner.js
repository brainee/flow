...
SQL.Row.prototype._build = function () {
    ...
    var td2 = OZ.DOM.elm("td", { className: "typehint", colSpan: 1 });
    var td5 = OZ.DOM.elm("td"); //by lshan

    //by ykn
    var td4 = OZ.DOM.elm("td");
    var del = _("del");
    this.dom.deleted = OZ.DOM.elm("div", { innerHTML: "<img alt=\'" + del + "\' src='images/del.png' Width='16' Height='16' />" });
    this.dom.title1 = OZ.DOM.elm("div");
    this.dom.deleted.style.display = "none";

    this.dom.typehint = td2;
    this.dom.colcontent = td5;
    this.dom.colcontent.style.display = "none"
    ...
    OZ.DOM.append(
		...
		[td1, this.dom.title],
        [this.dom.content, td5],  // by lshan
    //by ykn
        [this.dom.content, td4],
        [td4, this.dom.deleted, this.dom.title1]
    ...
    OZ.Event.add(this.dom.deleted, "click", this.bind(this.DeleteR));
}
...
//    $("#searchDiv").remove();
    ...
    $("#bar").css("display", "");
    if (this.data.title != "NewColumn") {
        if (this.owner.rows.length == 1)
            this.owner.addRow("NewColumn", { ai: false });
        else {
            var isExist = false;
            for (var i = 0; i < this.owner.rows.length; i++) {
                if (this.owner.rows[i].data.title == "NewColumn")
                    isExist = true;
            }
            if (!isExist)
                this.owner.addRow("NewColumn", { ai: false });
...
SQL.Row.prototype.buildEdit = function () {
    ...
    //add by  zwy
//    OZ.Event.add(this.dom.name, "keyup", this.columnkeyup);
    ...
    this.dom.comment = OZ.DOM.elm("input");
    this.dom.comment.type = "text";
    elms.push(["comment", this.dom.comment]);
    ...
    for (var i = 0; i < elms.length; i++) {
        ...
        var td1 = OZ.DOM.elm("td");
        var td2 = OZ.DOM.elm("td", { colSpan: 2 });
        var td3 = OZ.DOM.elm("td", { colSpan: 2 });
...
//    OZ.Event.add(this.dom.comment, "keyup", this.commentkeyup);

}
//??????????????
//function clickS(ids) {
//    $(currentSearchObject).val($("#" + ids + " B").html());
//    $("#searchDiv").remove();
//}

//var currentSearchObject;
//SQL.Row.prototype.commentkeyup = function (e) {
//    currentSearchObject = this;
//    if (e.keyCode != "38" && e.keyCode != "40")//?????????????????????Document??keyup????
//    {
//        pointRow = -1; //???????????????pointRow?????????txtSearch?????????????????????????????pointRow??????
...
//        //??????????????????
//        if ($(this).val() != "") {
//            var waiting = "<img src='images/loading.gif'/>&nbsp;&nbsp;loading..."; //???????????????????
//            searchDiv(leftS, topS, widthS, waiting); //??????????????
...
//                             }
//                             else {
//                                 setTimeout(function () { //??????????????????????????????0.3???????0.3???????????????????????????????????????????...
//                                     $("#searchDiv").remove(); //??????????????
//                                     searchDiv(leftS, topS, widthS, data); //????????????????????
//                                 }, 100);
//                             }
//                         }
//                         catch (e) {
...
//    }
//}
//SQL.Row.prototype.columnkeyup = function (e) {
//    currentSearchObject = this;
//    if (e.keyCode != "38" && e.keyCode != "40")//?????????????????????Document??keyup????
//    {
//        pointRow = -1; //???????????????pointRow?????????txtSearch?????????????????????????????pointRow??????
...
SQL.Row.prototype.collapse = function () {
    ...
    var data = {
        ...
        comment: this.dom.comment.value//add by zwy
    }
    ...
    var renameColumn = this.data.title; //add by  wyzhang
    if (editColumn != "") {
        if ((editColumn == "NewColumn" && this.owner.rows.length == 1)
    || (renameColumn != editColumn && editColumn != "NewColumn")) { //rename column
            var tbName = this.owner.data.title.split('(')[0];

            for (var i = 0; i < allIndex.length; i++) {

                if (allIndex[i].tablename == tbName) {

                    var keyColumns = allIndex[i].keyColumns;
                    if (keyColumns == "NewColumn") {
                        allIndex[i].keyColumns = renameColumn;
                        allIndex[i].IncludeColumns = "";
                    }
                    else {
                        var includeColumns = allIndex[i].IncludeColumns;

                        var tempkeyColumns = ',' + keyColumns + ',';
                        var tempincludeColumns = ',' + includeColumns + ',';
                        var cureditColumn = ',' + editColumn + ',';
                        renameColumn = ',' + renameColumn + ',';
                        tempkeyColumns = tempkeyColumns.replace(cureditColumn, renameColumn);
                        tempincludeColumns = tempincludeColumns.replace(cureditColumn, renameColumn);

                        allIndex[i].keyColumns = tempkeyColumns.substr(1, tempkeyColumns.length - 2);
                        if (includeColumns!="")
                        allIndex[i].IncludeColumns = tempincludeColumns.substr(1, tempincludeColumns.length - 2);
                    }
                }
            }
        }
...
SQL.Row.prototype.redraw = function () {
    ...
    var tbName = this.owner.data.title.split('(')[0]; //add by wyzhang
    if (this.data.title != "");
    {
        var colName = this.data.title.split('(')[0];
        var primaryColumn = '';
        for (var i = 0; i < allIndex.length; i++) {
            if (allIndex[i].tablename == tbName && allIndex[i].type == "PRIMARY") {
                primaryColumn = allIndex[i].keyColumns;
                break;
            }
        }
        if (primaryColumn != "") {
            var tempPrimaryColumn = ',' + primaryColumn + ','
            var currentColumn = ',' + colName + ','; 
            if (tempPrimaryColumn.lastIndexOf(currentColumn) > -1) { OZ.DOM.addClass(this.dom.title, "primary"); }
        }
    }
    ...
    this.dom.colcontent.innerHTML = this.data.comment.substring(0,10);
    // by ?ykn
    if (this.data.title != "NewColumn" && this.data.title != "")
        this.dom.deleted.style.display = (this.selected ? "" : "none");
    //	if (this.expanded)
    //	    this.dom.container.title ='??delete????????????'+this.data.comment;
    //    else
    //        this.dom.container.title = '???????,?????delete????????????' + this.data.comment;
...
SQL.Row.prototype.toXML = function () {
    ...
    if ($.trim(t) != "NewColumn") {
        ...
        if (t.indexOf("(") > -1) {
            temptype = t.split("(")[0];
            //'binary','char','varchar','varbinary','nchar','nvarchar' 
            if (temptype == "binary" || temptype == "char" || temptype == "varchar" ||
	                temptype == "nchar" || temptype == "varbinary" || temptype == "nvarchar"
                    || temptype == "decimal" || temptype == "numeric"
                     )
                columninfo = "[" + t.replace("(", "](");
            else
                columninfo = "[" + temptype + "]";
        }
        else {
        ...
        xml += "<columninfo>" + columninfo + "</columninfo>\n";
        ...
        if (this.data.def || this.data.def === null) {
            ...
            if (d === null) {
                ...
                var defval = this.data.def;
                //                defval = defval.replace(/>/g, "&gt;").replace(/</g, "&lt;");
                if (temptype == "char" || temptype == "varchar" || temptype == "xml" || temptype == "varchar"
		            || temptype == "nchar" || temptype == "nvarchar" || temptype == "text" || temptype == "ntext") {
                    if (CheckInt(defval, "^-?\\d+$")) {
                        defval = "(" + defval + ")";
                    }
                    else if (defval == "''") {
                        defval = "'" + defval + "'";
                    }
                    else if (defval.charAt(0) == "'" && defval.charAt(defval.length - 1) == "'") {
                ...
                else if ((temptype == "datetime" || temptype == "smalldatetime") && (d == '' || d == 'getdate()'))
                    sqldefault = "'(getdate())'";
                else if (temptype == "datetime" || temptype == "smalldatetime") {
                    sqldefault = "'('" + d + "')'";
                }
                else if (temptype == "uniqueidentifier")
                    sqldefault = "'(newid())'";
                else if (temptype == "smallint" || temptype == "bigint" || temptype == "int" || temptype == "monetary" || temptype == "money"
		                || temptype == "smallmoney" || temptype == "float" || temptype == "decimal" || temptype == "numeric")
                    sqldefault = "'((" + d + "))'";
                else if (temptype == "varbinary" || temptype == "binary") {
                    sqldefault = "(" + d + ")";
                }
                else
                    sqldefault = q + d + q;
            ...
            sqldefault = sqldefault.replace(/>/g, "&gt;").replace(/</g, "&lt;");
            xml += "<sqldefault>" + sqldefault + "</sqldefault>";
        }
        else {
            xml += "<default></default>";
            xml += "<sqldefault>9999</sqldefault>";
        }
        ...
        if (this.data.comment) {
            ...
            //deal key add by wyzhang 20121207
            var tempescaped = escaped.toLowerCase();
            var i = tempescaped.lastIndexOf("insert");
            var s = tempescaped.lastIndexOf("select");
            if (i > -1)
                escaped = escaped.substring(0, i) + "[" + escaped.substring(i, i + 6) + "]" + escaped.substr(i + 6, escaped.length - i - 6)
            if (s > -1)
                escaped = escaped.substring(0, s) + "[" + escaped.substring(s, s + 6) + "]" + escaped.substr(s + 6, escaped.length - s - 6)
            ...
            xml += "<sqlcomment>" + escaped + "</sqlcomment>\n";
...
SQL.Relation.prototype.init = function (owner, row1, row2) {
    ...
    this.owner.io.openWin();
    this.owner.io.updatetextarea();
}
...
SQL.Table.prototype._build = function () {
    this.dom.container = OZ.DOM.elm("div", { className: "table" }); //add by lshan
    this.dom.content = OZ.DOM.elm("table");
    var thead = OZ.DOM.elm("thead");
    var tr = OZ.DOM.elm("tr"); //
    this.dom.title = OZ.DOM.elm("td", { className: "title", colSpan: 3 });
    //this.dom.deleted = OZ.DOM.elm("div", { className: "selected", innerHTML: "<img alt=\"???\" src='images/ball_stop ????.gif' />" });

    //    var td = OZ.DOM.elm("td");
    //    var td2 = OZ.DOM.elm("td");

    //    var td3 = OZ.DOM.elm("td");
    //    var td4 = OZ.DOM.elm("td");
    //    var td5 = OZ.DOM.elm("td");
    //    this.dom.title1 = OZ.DOM.elm("div");

    var del = _("del");
    var copy = _("copy");
    this.dom.deleteT = OZ.DOM.elm("td", { innerHTML: "<img alt=\'" + del + "\' src='images/ORB_Icons_del.png' title=\'" + del + "\' />" });
    //    this.dom.title2 = OZ.DOM.elm("div");
    this.dom.copyBtn = OZ.DOM.elm("td", { innerHTML: "<img alt=\'" + copy + "\' src='images/ORB_Icons_add.png' title=\'" + copy + "\' />" });
    OZ.DOM.append(
		[this.dom.container, this.dom.content],
    ...
    //    //        [tr, td3, td4],
    //            [tr, this.dom.copyBtn],
    //    //        [td3, this.dom.copyBtn, this.dom.title2],
    //    //[tr, td, td2],
    //    //        [td4, this.dom.deleteT, this.dom.title1]
    //            [tr, this.dom.deleteT]
	);
    ...
    this._ec.push(OZ.Event.add(this.dom.deleteT, "click", this.bind(this.DeleteT)));
    this._ec.push(OZ.Event.add(this.dom.copyBtn, "click", this.bind(this.CopyBtn)));
    this._ec.push(OZ.Event.add(this.dom.container, "mouseover", this.bind(this.mouseover))); //add by lshan
    this._ec.push(OZ.Event.add(this.dom.container, "mouseout", this.bind(this.mouseout))); //add by lshan
//    this._ec.push(OZ.Event.add(window, "mouseup", this.bind(this.mouseup)));
}
// by ????????
// ???????
SQL.Table.prototype.DeleteT = function (t) {
...
SQL.Table.prototype.CopyBtn = function (t) {
    var old = this.dom.title.innerHTML;
    //    var old = this.dom.container.find("table thead").eq(0).find("tr").eq(0).find("td").eq(0).html();
    this.owner.io.updatetextarea();
    for (var i = 0; i < this.owner.tables.length; i++) {
        if (this.owner.tables[i].data.title == old) {
            var result = confirm(_('copyTable'));
            if (!result) {
                return;
            }
            var xml = this.owner.tables[i].toXML()
            var resultXml = "";
            ...
            $("#designFrm", parent.document).contents().find("#clientload").click(); //????xml
            break;
        }
...
//add by lshan
SQL.Table.prototype.ColDesHiddenOrDisplay = function () {
    var div = this.dom.container;
    var tbody = div.getElementsByTagName("tbody");
    if (tbody[0].getElementsByTagName("td")[2].style.display == "none") {
...
//add by lshan
SQL.Table.prototype.ColHiddenOrDisplay = function () {
    var div = this.dom.container;
    var tbody = div.getElementsByTagName("tbody");
    if (tbody[0].getElementsByTagName("td")[0].style.display == "none") {
        for (var i = 0; i < tbody.length; i++) {
...
var ColDesbtnid = 0;
var timer = null;
SQL.Table.prototype.mouseover = function (e) {
    if (timer) clearTimeout(timer);
    if (this.checkHover(e, this.dom.container)) {
        var div = this.dom.container;
        var tools = document.getElementById("tools");
        var top, left;
        var z_index = div.style.zIndex;
        if (div.currentStyle) {
            left = div.currentStyle.left; top = div.currentStyle.top;
        }
        else if (window.getComputedStyle) {
            left = window.getComputedStyle(div, null).left; top = window.getComputedStyle(div, null).top;
        }
        if (copybtnid != 0) {
            OZ.Event.remove(copybtnid);
        }
        if (delTbtnid != 0) {
            OZ.Event.remove(delTbtnid);
        }
        if (Colbtnid != 0) {
            OZ.Event.remove(Colbtnid);
        }
        if (ColDesbtnid != 0) {
            OZ.Event.remove(ColDesbtnid);
        }
        copybtnid = OZ.Event.add(tools.getElementsByTagName("img")[2], "click", this.bind(this.CopyBtn));
        delTbtnid = OZ.Event.add(tools.getElementsByTagName("img")[3], "click", this.bind(this.DeleteT));
        Colbtnid = OZ.Event.add(tools.getElementsByTagName("img")[0], "click", this.bind(this.ColHiddenOrDisplay));
        ColDesbtnid = OZ.Event.add(tools.getElementsByTagName("img")[1], "click", this.bind(this.ColDesHiddenOrDisplay));
        tools.style.zIndex = z_index;
        tools.style.left = parseInt(left) + div.offsetWidth - 76 + "px";
        tools.style.top = (parseInt(top) - 20) + "px";
        tools.style.display = "";
        //var test = div.getElementsByTagName("tbody")[0].getElementsByTagName("td")[2].style.display;
    }
}

//add by lshan
SQL.Table.prototype.mouseout = function (e) {
    if (this.checkHover(e, this.dom.container)) {
        var tools = document.getElementById("tools");
...
//add by lshan
SQL.Table.prototype.contains = function (parentNode, childNode) {
...
//add by lshan
SQL.Table.prototype.checkHover = function (e, target) {
    if (this.getEvent(e).type == "mouseover") {
        return !this.contains(target, this.getEvent(e).relatedTarget || this.getEvent(e).fromElement) && !((this.getEvent(e).relatedTarget || this.getEvent(e).fromElement) === target);
    }
    else {
        return !this.contains(target, this.getEvent(e).relatedTarget || this.getEvent(e).toElement) && !((this.getEvent(e).relatedTarget || this.getEvent(e).toElement) === target);
    }
}

//add by lshan
SQL.Table.prototype.getEvent = function (e) {
...
SQL.Table.prototype.dblclick = function (e) {
    editTable = this.data.title.split('(')[0];
    var t = OZ.Event.target(e);
...
SQL.Table.prototype.addRow = function (title, data) {
    ...
    this.redraw();
//    if (this.rows.length == 1 && this.rows[0].data.title == "NewColumn") {//add by wyzhang
//        var tempIndex = new IndexBase(); 
//        tempIndex.tablename = this.data.title;
//        tempIndex.name = "PK_" + this.data.title;
//        tempIndex.type = "PRIMARY";
//        tempIndex.unique = "1";
//        tempIndex.keyColumns = "NewColumn";
//        tempIndex.IncludeColumns = "";
...
var oldrow;
var oldrowcolor;
var selectdivcolor = "#FFCCFF";
var selectrowcolor = "lavender";
var selectrow = false;
var mouseX, mouseY;
var divX, divY;
...
SQL.Table.prototype.down = function (e) { /* mousedown - start drag */
    ...
    if (t != this.dom.title) {
        //                this.owner.tableManager.select(this);
        //        this.owner.rowManager.select(this);
        //        this.owner.rowManager.rowClick(e);
        var div = this.dom.container;
        var pos = new Array();
        ...
        mouseX = e.pageX || e.clientX + scrollLeft - document.body.clientLeft;
        mouseY = e.pageY || e.clientY + scrollTop - document.body.clientTop;
        //        for (var j = 0; j < t.parentNode.children.length; j++) {
        //                        if (t.parentNode.children[j].children[0].children[0].innerHTML == t.children[0].children[0].innerHTML) {
        //                            RowOrigin = new SQL.Row(this, this.rows[j - 1].data.title, this.rows[j - 1].data);
        //                            this.owner.rowManager.select(this.rows[j - 1]);
        //                        }
        //            var pos2 = getPosition(t.parentNode.children[j]);
        //            if (e.clientX > pos2[1] && e.clientX < pos2[1] + t.parentNode.children[j].offsetWidth && e.clientY > pos2[0] && e.clientY < pos2[0] + t.parentNode.children[j].offsetHeight) {
        //                RowOrigin = new SQL.Row(this, this.rows[j - 1].data.title, this.rows[j - 1].data);
        //                RowOrigin.dispatch("rowclick", RowOrigin);
        //                this.owner.rowManager.select(this.rows[j - 1]);
        //            }
        //        }
        timer = setTimeout(function () {
            selecttab.owner.tableManager.select(selecttab);
            for (var k = 0; k < selecttab.rows.length; k++) {
                if (selecttab.rows[k].expanded == true) {
                    return;
                }
            }
            var z_index = div.style.zIndex;
            for (var j = 0; j < t.parentNode.children.length; j++) {
                ...
                //            }
                var pos2 = getPosition(t.parentNode.children[j]);
                if (mouseX > pos2[1] && mouseX < pos2[1] + t.parentNode.children[j].offsetWidth && mouseY > pos2[0] && mouseY < pos2[0] + t.parentNode.children[j].offsetHeight) {
                    RowOrigin = new SQL.Row(selecttab, selecttab.rows[j - 1].data.title, selecttab.rows[j - 1].data);
                    RowOrigin.dispatch("rowclick", RowOrigin);
                    selecttab.owner.rowManager.select(selecttab.rows[j - 1]);
                    t = t.parentNode.children[j];
                }
            }
            if (t.children[0].children[0].children[0].innerHTML != "NewColumn") {
                selectedtablediv = t.parentNode.parentNode;
                oldrow = t;
                oldrowcolor = t.style.backgroundColor;
                temprow.innerHTML = "<table id = 'temptab'><TBODY>" + t.innerHTML + "</TBODY></table>";
                var temptab = document.getElementById("temptab");
                temptab.style.backgroundColor = selectdivcolor;
                t.style.backgroundColor = selectrowcolor;
                temptr = temptab.children[0].children[0];
                temptr.style.height = t.children[0].offsetHeight;
                //                var aaa = t.parentNode.children.length;
                for (var i = 0; i < temptr.children.length; i++) {
                    if (t.children[0].children[i].offsetWidth != 0) {
                        temptr.children[i].width = t.children[0].children[i].offsetWidth;
                    }
                }
                temprow.style.top = pos[0] + "px";
                temprow.style.left = pos[1] + "px";
                temprow.style.zIndex = z_index + 1;
...
SQL.Table.prototype.getPos = function(cell) {
    var pos = new Array();
...
SQL.Table.prototype.toXML = function () {
    ...
    var xml = "";
    if ($.trim(t) != "TableName") {
        var realTableName = $.trim(t);
        if (t.indexOf("(") > -1)
            realTableName = t.substr(0, t.indexOf("("));
        xml += '<table x="' + this.x + '" y="' + this.y + '" name="' + $.trim(t) + '" realtablename="' + realTableName + '">\n';
        for (var i = 0; i < this.rows.length; i++) {
        ...
        for (var m = 0; m < allIndex.length; m++) {
            if (allIndex[m].tablename == $.trim(t).split('(')[0]) {
                if (allIndex[m].type == "PRIMARY")
                    xml += '<key sequence="' + (startSequence + 1) + '"   type="' + allIndex[m].type + '"  is_primary_key="1"   is_unique="' + allIndex[m].unique + '" name="' + allIndex[m].name + '">\n';
                else
                    xml += '<key sequence="' + (startSequence + 1) + '"   type="' + allIndex[m].type + '"  is_primary_key="0"   is_unique="' + allIndex[m].unique + '" name="' + allIndex[m].name + '">\n';
                if (allIndex[m].keyColumns != "") {
                    var keyColumnArray = allIndex[m].keyColumns.split(',');
                    for (var h = 0; h < keyColumnArray.length; h++) {
                        xml += '<part sequence="' + (h + 1) + '" key_ordinal="1" is_included_column="0"   columnName="' + keyColumnArray[h] + '">' + keyColumnArray[h] + '</part>\n';
                    }
                }
                
                if (allIndex[m].IncludeColumns != "") {
                    var includeColumnArray = allIndex[m].IncludeColumns.split(',');
                    for (var h = 0; h < includeColumnArray.length; h++) {
                        //                        xml += '<part sequence="' + (h + 1) + '" columnName="' + includeColumnArray[h] + '" IndexColumnType= "include">' + includeColumnArray[h] + '</part>\n';
                        xml += '<part sequence="' + (h + 1) + '" key_ordinal="0" is_included_column="1"   columnName="' + includeColumnArray[h] + '">' + includeColumnArray[h] + '</part>\n';
                    }
                }
                xml += '</key>\n';
                startSequence++;
        ...
        if (c) {
            //deal key add by wyzhang 20121207
            var tempescaped = c.toLowerCase();
            var i = tempescaped.lastIndexOf("insert");
            var s = tempescaped.lastIndexOf("select");
            if (i > -1)
                c = c.substring(0, i) + "[" + c.substring(i, i + 6) + "]" + c.substr(i + 6, c.length - i - 6)
            if (s > -1)
                c = c.substring(0, s) + "[" + c.substring(s, s + 6) + "]" + c.substr(s + 6, c.length - s - 6)
...
var allIndex = [];
function IndexBase() {
    this.keyColumns.length = 0;
    this.IncludeColumns.length = 0;
};
IndexBase.prototype.tablename = '';
IndexBase.prototype.name = '';
IndexBase.prototype.type = '';
IndexBase.prototype.keyColumns = '';
IndexBase.prototype.IncludeColumns = '';
IndexBase.prototype.unique = '';
IndexBase.prototype.showColor = function () {
    alert(this.color);
}
...
SQL.Table.prototype.fromXML = function (node) {
    var name = node.getAttribute("name");
    this.setTitle(name);
    var x = parseInt(node.getAttribute("x")) || 0 + 20;
    var y = parseInt(node.getAttribute("y")) || 0 + 20;
    this.moveTo(x, y);
    ...
    for (var i = 0; i < allIndex.length; ) {//add by wyzhang
        if (allIndex[i].tablename == name.split('(')[0]) {
            allIndex.splice(i, 1);
        }
        else
            i++;
    ...
    for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        var tempIndex = new IndexBase();
        tempIndex.tablename = name.split('(')[0];
        tempIndex.name = key.getAttribute("name");
        tempIndex.type = key.getAttribute("type");
        tempIndex.unique = key.getAttribute("is_unique");
        var parts = key.getElementsByTagName("part"); // key.childNodes;
        for (var j = 0; j < parts.length; j++) {
            if (parts[j].getAttribute("key_ordinal") != null && parts[j].getAttribute("key_ordinal") != "" && parts[j].getAttribute("key_ordinal") == "1")
                tempIndex.keyColumns += parts[j].firstChild.nodeValue + ',';
            else
                tempIndex.IncludeColumns += parts[j].firstChild.nodeValue + ',';
        }
        if (tempIndex.keyColumns.length > 0)
            tempIndex.keyColumns = tempIndex.keyColumns.substr(0, tempIndex.keyColumns.length - 1);
        if (tempIndex.IncludeColumns.length > 0)
            tempIndex.IncludeColumns = tempIndex.IncludeColumns.substr(0, tempIndex.IncludeColumns.length - 1)
        allIndex.push(tempIndex);
    }
...
SQL.Key.prototype.toXML = function (keySequence) {
    var xml = "";
    xml += '<key sequence="' + (keySequence + 1) + '"   type="' + this.getType() + '" name="' + this.getName() + '">\n';
    for (var i = 0; i < this.rows.length; i++) {
        var r = this.rows[i];
        xml += '<part sequence="' + (i + 1) + '" columnName="' + r.getTitle() + '" >' + r.getTitle() + '</part>\n';
    }
    xml += '</key>\n';
...
SQL.IO.prototype.init = function (owner) {
    ...
    var ids = ["saveload", "clientImport", "Save", "prevsave", "Open", "Release", "clientload", "clientsql",
                 "serverimport", "clientsave", "updatetextarea", "Export", "OpenER", "SaveER", "btnBestDesgin", "ExportScript"]; // "quicksave", "serversave", "serverload", "serverlist","",
    ...
    OZ.Event.add(this.dom.saveload, "click", this.bind(this.saveload));
    OZ.Event.add(this.dom.Export, "click", this.bind(this.Export));

    OZ.Event.add(this.dom.clientImport, "click", this.bind(this.clientImport));

    OZ.Event.add(this.dom.Save, "click", this.bind(this.Save));
    OZ.Event.add(this.dom.prevsave, "click", this.bind(this.prevsave));

    OZ.Event.add(this.dom.Open, "click", this.bind(this.Open));
    OZ.Event.add(this.dom.btnBestDesgin, "click", this.bind(this.ShowBestDesign));

    OZ.Event.add(this.dom.OpenER, "click", this.bind(this.OpenER));

    OZ.Event.add(this.dom.SaveER, "click", this.bind(this.SaveER));

    OZ.Event.add(this.dom.ExportScript, "click", this.bind(this.ExportScript));

    OZ.Event.add(this.dom.Release, "click", this.bind(this.Release));

    OZ.Event.add(this.dom.openWin, "click", this.bind(this.openWin));

    OZ.Event.add(this.dom.clientsave, "click", this.bind(this.clientsave));
    OZ.Event.add(this.dom.updatetextarea, "click", this.bind(this.updatetextarea));
...
SQL.IO.prototype.clientImport = function () { /* open io dialog */
    this.serverimport(this);
}
function CheckInt(size, rule) { 
    var re = new RegExp(rule);
    if (size.match(re) == null)
        return false;
    else
        return true;
}

SQL.IO.prototype.prevsave = function () { /*click save button ,check validate before save add by wyzhang */
    var tableCount = this.owner.tables.length;
    var validate = true;
    if (tableCount == 0) {
        alert(_("notable"));
    ...
    else if (tableCount == 1 && this.owner.tables[0].data.title == "TableName") {
        alert(_("tablename"));
        $("#background").css("visibility", "hidden");
        $("#savedesign").css("display", "none");
        return;
    ...
    for (var i = 0; i < tableCount; i++) {
        var tableName = this.owner.tables[i].data.title.split("(")[0];
    ...
    for (var i = 0; i < allTable.length; i++) {
        var temp = allTable[i];
        for (var j = i + 1; j < allTable.length; j++) {
            if (allTable[j].toUpperCase() == temp.toUpperCase()) {
                repeat = true;
            }
        }
    }
    ...
    for (var i = 0; i < tableCount; i++) {
        var thisTable = this.owner.tables[i];
        var aiDef = false;
        var aiCount = 0;
        for (var j = 0; j < thisTable.rows.length; j++) {

            if (thisTable.rows[j].data.ai) {
                aiCount++;
            }
            if (thisTable.rows[j].data.ai && thisTable.rows[j].data.def != "") {
                aiDef = true;
            }
        }
        if (aiDef) {
            alert(thisTable.data.title + _("noaidef"));
            $("#background").css("visibility", "hidden");
            $("#savedesign").css("display", "none");
            return;
        }
        if (aiCount > 1) {
            alert(thisTable.data.title + _("toomanyai"));
            $("#background").css("visibility", "hidden");
            $("#savedesign").css("display", "none");
            return;
        }
    ...
    for (var i = 0; i < tableCount; i++) {
        ...
        for (var j = 0; j < thisTable.rows.length; j++) {
            tbColumns.push(thisTable.rows[j].data.title.toUpperCase())
        }

        for (var m = 0; m < tbColumns.length; m++) {
            var temp = tbColumns[m];
            for (var n = m + 1; n < tbColumns.length; n++) {
                if (tbColumns[n] == temp) {
                    repeat = true;
                }
            }
    ...
    for (var i = 0; i < tableCount; i++) {
        var tbComment = this.owner.tables[i].data.comment;
        if ($.trim(tbComment) == "") {
            alert(this.owner.tables[i].data.title + '' + _("noTableCommont"));
            $("#background").css("visibility", "hidden");
            $("#savedesign").css("display", "none");
            return;
        }
        if (this.owner.tables[i].data.title == "TableName") {
            alert(_("tablename"));
            $("#background").css("visibility", "hidden");
            $("#savedesign").css("display", "none");
            return;
    ...
    for (var i = 0; i < tableCount; i++) {
        ...
        var primaryCount = 0;
        var primaryColumns = '';
        for (var k = 0; k < allIndex.length; k++) {
            if (allIndex[k].tablename == thisTable.data.title.split('(')[0] && allIndex[k].type == "PRIMARY") {
                isSetPrimary = true;
                primaryCount++;
                primaryColumns = allIndex[k].keyColumns;
            }
        }
        if (!isSetPrimary) {
            alert(thisTable.data.title + _("noprimary"));
            $("#background").css("visibility", "hidden");
            $("#savedesign").css("display", "none");
            return;
        }
        for (var j = 0; j < thisTable.rows.length; j++) {
            var curCol = ',' + thisTable.rows[j].data.title + ',';
            var tempprimaryColumns = ',' + primaryColumns + ',';
            if (tempprimaryColumns.lastIndexOf(curCol) > -1 && thisTable.rows[j].data.nll) {
                alert(thisTable.data.title + _("primarynll"));
                return;
            }
        }
        if (primaryCount > 1) {
            alert(thisTable.data.title + _("toomanyprimary"));
            $("#background").css("visibility", "hidden");
            $("#savedesign").css("display", "none");
            return;
        }
    ...
    for (var i = 0; i < tableCount; i++) {
        var thisTable = this.owner.tables[i];
        if (thisTable.rows.length < 1 || (thisTable.rows.length == 1 && thisTable.rows[0].data.title == "NewColumn")) {
            alert(thisTable.data.title + _("nocloumn"));
            $("#background").css("visibility", "hidden");
            $("#savedesign").css("display", "none");
            return;
        }
        ...
        for (var j = 0; j < thisTable.rows.length; j++) {
            allrowname += thisTable.rows[j].data.title + ',';
            if (thisTable.rows[j].data.title != "NewColumn") {

                if (thisTable.rows[j].data.type == 0) {//
                    validate = false;
                    thisTable.rows[j].expand();
                    thisTable.rows[j].dom.type.style.border = "solid 2px #F00";
                }
                else if (thisTable.rows[j].data.type == 8 || thisTable.rows[j].data.type == 9) {//('decimal','numeric')  
                    var cSize = thisTable.rows[j].data.size;
                    var arrSize = cSize.split(',');

                    if ($.trim(cSize) == "" || arrSize.length != 2) {//??? ???????','
                        validate = false;
                        thisTable.rows[j].expand();
                        thisTable.rows[j].dom.size.style.border = "solid 2px #F00";
                    }
                    else {
                        if (!CheckInt(arrSize[0], "^\\d+$") || !CheckInt(arrSize[1], "^\\d+$")) {//?????????? ???>=0
                            validate = false;
                            thisTable.rows[j].expand();
                            thisTable.rows[j].dom.size.style.border = "solid 2px #F00";
                        }
                        else {
                            if (parseInt(arrSize[0]) < parseInt(arrSize[1])) {
                                validate = false;
                                thisTable.rows[j].expand();
                                thisTable.rows[j].dom.size.style.border = "solid 2px #F00";
                            }
                            else if (parseInt(arrSize[0]) > 38) {
                                validate = false;
                                thisTable.rows[j].expand();
                                thisTable.rows[j].dom.size.style.border = "solid 2px #F00";
                            }
                ...
                else if (thisTable.rows[j].data.type == 10 || thisTable.rows[j].data.type == 11
                //                        || thisTable.rows[j].data.type == 13 
                        || thisTable.rows[j].data.type == 15 || thisTable.rows[j].data.type == 16
                        || thisTable.rows[j].data.type == 20 || thisTable.rows[j].data.type == 21
                        ) {//('char','varchar','XML','nchar','nvarchar','binary','varbinary',10 11 13  15 16 20 21

                    var cSize = thisTable.rows[j].data.size;
                    if (!CheckInt(cSize, "^[0-9]*[1-9][0-9]*$") && $.trim(cSize.toUpperCase()) != "MAX") {
                        validate = false;
                        thisTable.rows[j].expand();
                        thisTable.rows[j].dom.size.style.border = "solid 2px #F00";
                    }
                }
                else if (thisTable.rows[j].data.type < 5 && thisTable.rows[j].data.type > 0) {// int  -  money
                    if ($.trim(thisTable.rows[j].data.def) != "") {//?????????
                        if (!CheckInt(thisTable.rows[j].data.def, "^-?\\d+$")) {//????????????
                            validate = false;
                            thisTable.rows[j].expand();
                            thisTable.rows[j].dom.def.style.border = "solid 2px #F00";
                        }
                    }
                }
                else if (thisTable.rows[j].data.type < 10 && thisTable.rows[j].data.type > 6) {//float number 
                    if ($.trim(thisTable.rows[j].data.def) != "") {
                        if (!CheckInt(thisTable.rows[j].data.def, "^(-?\\d+)(\\.\\d+)?$")) {
                            validate = false;
                            thisTable.rows[j].expand();
                            thisTable.rows[j].dom.def.style.border = "solid 2px #F00";
                        }
                ...
                if ($.trim(thisTable.rows[j].data.comment) == "") {//
                    validate = false;
                    thisTable.rows[j].expand();
                    thisTable.rows[j].dom.comment.style.border = "solid 2px #F00";
                }
            }

        }

        var IndexColumnNotInTable = false;
        for (var p = 0; p < allIndex.length; p++) {
            if (allIndex[p].tablename == thisTable.data.title.split('(')[0]) {
                var keycolumn = allIndex[p].keyColumns;
                var includecolumn = allIndex[p].IncludeColumns;

                for (var q = 0; q < keycolumn.split(',').length; q++) {
                    var tempkey = ',' + keycolumn.split(',')[q] + ',';
                    if (allrowname.lastIndexOf(tempkey) < 0) {
                        IndexColumnNotInTable = true;
                        break;
                    }
                }
                if (includecolumn != "") {
                    for (var q = 0; q < includecolumn.split(',').length; q++) {
                        var tempInclude = ',' + includecolumn.split(',')[q] + ',';
                        if (allrowname.lastIndexOf(tempInclude) < 0) {
                            IndexColumnNotInTable = true;
                            break;
                        }
                    }
        ...
        if (IndexColumnNotInTable) {
            alert(thisTable.data.title + _("IndexColumnNotInTable"));
            return;
        }
    }
    ...
    $(window.frames["SaveSoluationFrame"].document).find("#gdvDesignTables").find("tbody").html("");
    $(window.frames["SaveSoluationFrame"].document).find("#chkPartition").attr("checked", false);

    $(window.frames["SaveSoluationFrame"].document).find("#gdvTVPTables").find("tbody").html("");
    $(window.frames["SaveSoluationFrame"].document).find("#chkTVP").attr("checked", false);



    $("#background").css("visibility", "visible");
    $("#savedesign").css("display", "block");

    if ($("#HFDesignDBName").val() != "-1") {
        $(window.frames["SaveSoluationFrame"].document).find("#ddlDBName").val($("#HFDesignDBName").val()); //ddlDBmy_input
    }
    $(window.frames["SaveSoluationFrame"].document).find("#txtReleaseVersion").val($("#HFOpenDesignName").val());
    // save  list of table name , ready for partition ,add by wyzhang 2012-08-31
    $(window.frames["SaveSoluationFrame"].document).find("#HFTableNameList").val(allTable.join(','));
}

SQL.IO.prototype.Save = function () { /* save design add  by wyzhang */
    if ($.trim($(window.frames["SaveSoluationFrame"].document).find("#txtReleaseVersion").val()) == "") {
        $(window.frames["SaveSoluationFrame"].document).find("#txtReleaseVersion").focus();
        return;
    }
    if ($(window.frames["SaveSoluationFrame"].document).find("#ddlDBName").val() == "-1") {
        $(window.frames["SaveSoluationFrame"].document).find("#ddlDBName").focus();
        return;
    }
    this.clientsql();
}

SQL.IO.prototype.Export = function () {//???????
    var xml;
    try {
...
SQL.IO.prototype.saveload = function () { /* open io dialog ?? ????/????  ???*/
    this.build();
    this.dom.ta.value = "";
    ...
    this.updatetextarea();
    this.owner.window.dom.background.style.visibility = "hidden";
    this.owner.window.dom.container.style.visibility = "hidden";
}

SQL.IO.prototype.ShowBestDesign = function () { /* open io dialog ????????*/
   
    this.saveload();
    openwindow('BestDesignList.aspx', 'newwindow', 680, 500)
}

SQL.IO.prototype.Open = function () { /* open io dialog ????????*/
    //	this.build();
    //	this.dom.ta.value = "";
    //	this.dom.clientsql.value = _("clientsql") + " (" + window.DATATYPES.getAttribute("db") + ")";
    //	this.owner.window.open(_("saveload"),this.dom.container);
    ...
    //        this.updatetextarea();
    this.saveload(); //???? ?? ????????? 
    openwindow('DesignList.aspx?publish=0', 'newwindow', 680, 500);
//    window.open('DesignList.aspx?publish=0', '_blank');
}

SQL.IO.prototype.OpenER = function () {
    this.saveload();
    openwindow('OpenERPic.aspx', 'newwindow', 680, 500)
}

SQL.IO.prototype.SaveER = function () {
    this.clientsql('SaveER');
}

SQL.IO.prototype.ExportScript = function () {
    openwindow('ExportSQLScript.aspx', 'newwindow', 750, 550)
}

SQL.IO.prototype.Release = function () { /* ????DB???? */
    openwindow('DesignList.aspx?publish=1', 'newwindow', 680, 500)
}
...
SQL.IO.prototype.openWin = function () { /*  */
...
SQL.IO.prototype.updatetextarea = function () {
    var xml = this.owner.toSourceXML();
    $("#io tbody").find("tr").eq(1).find("td").find("textarea").val(xml);
    this.dom.ta.value = xml;
}
...
SQL.IO.prototype.clientsql = function (type) {
    ...
    if (type == null) {
        ...
        if ($("#HFServiceID").val() == '') {
            alert(_("missServiceID"));
            return;
        }
        OZ.Request(path, this.bind(this.finishOfSaveER), { xml: true });
    }
}
SQL.IO.prototype.finishOfSaveER = function (xslDoc) {
    this.owner.window.hideThrobber();
    var xml = this.owner.toXML();
    $.post("./DBHandler.ashx",
            {
                functionName: "SaveER",
                designUser: $("#HFServiceID").val(),
                dbName: $("#HFERDB").val(), 
                designXML: xml
            },
             function (data) {
                 try {
                     if (data == "-1") {
                         alert(_("SaveFailedTip"))
                     }
                     else if (data == "1") {
                         alert(_("SaveSuccessTip"))
                     }
                 }
                 catch (e) {
...
SQL.IO.prototype.finish = function (xslDoc) {
    this.owner.window.hideThrobber();
    var xmlSource = this.owner.toSourceXML();
    var sql = "";
    try {
        if (window.XSLTProcessor && window.DOMParser) {
            var parser = new DOMParser();
            var xmlDoc = parser.parseFromString(xmlSource, "text/xml");
    ...
    sql = sql.substr(0, sql.lastIndexOf("UNION ALL"));
    this.dom.ta.value = sql;
    var xml = this.owner.toXML();

    var partitionTableArray = [];
    var partitionSchemaIDArray = [];
    var partitionColumnArray = [];
    var TVPTableArray = [];
    if ($(window.frames["SaveSoluationFrame"].document).find("#chkPartition").attr("checked")) {
        $(window.frames["SaveSoluationFrame"].document).find("#gdvDesignTables").find("tbody").find("tr").each(function (i) {
            if (i > 0) {
                var partitionTable = $(this).find("td").eq(0).html();
                var schemaValue = $(this).find("td").eq(1).find("select").val();
                var partitionColumn = 'PartitionKey'; //$.trim($(this).find("td").eq(2).find("input").val());
                if (schemaValue != -1) {
                    partitionSchemaIDArray.push(schemaValue);
                    partitionColumnArray.push(partitionColumn);
                    partitionTableArray.push(partitionTable);
                }
            }
        });
    }
    ...
    if ($(window.frames["SaveSoluationFrame"].document).find("#chkTVP").attr("checked")) {
        $(window.frames["SaveSoluationFrame"].document).find("#gdvTVPTables").find("tbody").find("tr").each(function (i) {
            if (i > 0) {
                var tvpTable = $(this).find("td").eq(0).html();
                var tvpValue = $(this).find("td").eq(1).find("select").val();
                if (tvpValue == 1) {
                    TVPTableArray.push(tvpTable);
                }
            }
        });
    }
    ...
    //    var verson = $(window.frames["SaveSoluationFrame"].document).find("#txtReleaseVersionmy_input").val();
    //    var exist = false;
    //    if (verson == '--请输入--' || verson == '') {
    //        alert('请输入项目或需求变更名称');
    //        return false;
    //    }
    $(window.frames["SaveSoluationFrame"].document).find("#txtReleaseVersion").find("option").each(function () {
    ...
    //    if (!exist) {
    //        alert('您输入的项目或需求变更名称不存在，请重新输入');
    //        return false;
    //    }
    $.post("./DBHandler.ashx",
            {
                functionName: "Save",
                designName: escape($(window.frames["SaveSoluationFrame"].document).find("#txtReleaseVersion").val()), //+ verson), //$("#txtDesignName").val()
                designUser: $("#HFServiceID").val(),
                dbName: $(window.frames["SaveSoluationFrame"].document).find("#ddlDBName").val(), //$("#ddlDBName").val(),
                designXML: escape(xml),
                sqlScript: escape(sql),
                partitionTables: partitionTableArray.join(','),
                partitionSchemaIDs: partitionSchemaIDArray.join(','),
                partitionColumns: partitionColumnArray.join(','),
                tvpTables: TVPTableArray.join(',')
            },
             function (data) {
                 try {
                     if (data == "-1") {
...
SQL.IO.prototype.serverimport = function (e) {//?????????
    ...
    //   var xml = window.showModalDialog('ExportDB.aspx', '', 'edge:raised;resizable:yes;status:no;dialogWidth=350px;dialogHeight=300px;center=yes;help=no');
    //   
    //   if(xml){
    //        this.dom.ta.value =xml;
    //        this.clientload();
    ...
    if (tableCount > 0) {
        $("#HFHasTable").val("1");
        //        openwindow('ExportDB.aspx?has=1',  'newwindow1',263,550);

    }
    else {
        $("#HFHasTable").val("0");
        //       openwindow('ExportDB.aspx?has=0', 'newwindow1',263,550) 
    }
}
...
SQL.TableManager.prototype.init = function (owner) {
    ...
    var ids = ["addtable", "aligntables", "tablekeys","removetable"];
    for (var i = 0; i < ids.length; i++) {
        var id = ids[i];
        var elm = OZ.$(id);
        this.dom[id] = elm;
...
//    $("#searchDiv").remove();
    ...
    if (this.adding) {
        ...
        var r = newtable.addRow("NewColumn", { ai: false });
...
SQL.TableManager.prototype.remove = function (e) {
    ...
    for (var i = 0; i < titles.length; i++) {
        deltbNames += titles[i].getTitle().split('(')[0] + ',';
        titles[i] = "'" + titles[i].getTitle() + "'";
    }
    var allTbNames = ',';
    for (var i = 0; i < this.owner.tables.length; i++) {
        allTbNames += this.owner.tables[i].data.title.split('(')[0] + ',';
    }
    allTbNames = allTbNames.replace(deltbNames, ","); //exist tablename 
    ...
    //add by wyzhang
    if (allTbNames != "") {
        for (var i = 0; i < allIndex.length; ) {
            var curName = ',' + allIndex[i].tablename + ',';
            if (allTbNames.lastIndexOf(curName) < 0) {
                allIndex.splice(i, 1);
            }
            else
                i++;
...
SQL.TableManager.prototype.keys = function (e) { /* open keys dialog *///???????????????
    ...
    //add by wyzhang
    if (this.selection[0].rows.length < 1 || (this.selection[0].rows.length == 1 && this.selection[0].rows[0].data.title == "NewColumn")) {
        alert(this.selection[0].data.title + _("newcolumnIndex"));
        return;
    }
    ...
    var tableName = this.selection[0].data.title.split('(')[0];
    var allColumn = '';
    var allColumnType = '';
    var indexNameArray = [];

    var indexColumnArray = []; //key column??????column??||???
    var indexIncludeArray = []; //include ??
    var indexTypeArray = [];
    var indexUniqueArray = [];
    indexArray = allIndex;
    ...
    for (var i = 0; i < indexCount; i++) {
        ...
        if (k.tablename == tableName) {
            ...
            if (indexNameArray.length > 0) {

                var temp = ',' + indexNameArray.join(',') + ','
                if (temp.lastIndexOf(',' + k.name + ',') < 0) {
                    indexNameArray.push(k.name);
                    indexTypeArray.push(k.type);
                    indexUniqueArray.push(k.unique);
                    indexColumnArray.push(k.keyColumns);
                    indexIncludeArray.push(k.IncludeColumns);
                    tableIndexCount++;
                }
            }
            else {
    ...
    for (var i = 0; i < this.selection[0].rows.length; i++) {
        var r = this.selection[0].rows[i];
        var elm = r.getDataType();
        var t = elm.getAttribute("sql");
        if (r.data.title == "NewColumn") { continue; }
        allColumn += r.getTitle() + ",";
        allColumnType += t.split("(")[0] + ",";

    }
    if (allColumn.length > 0)
        allColumn = allColumn.substr(0, allColumn.length - 1);
    if (allColumnType.length > 0)
        allColumnType = allColumnType.substr(0, allColumnType.length - 1);
    var timstamp = (new Date()).valueOf();
    var retVal = window.showModalDialog('IndexManager.aspx?tablename=' + tableName
     + '&allColumn=' + allColumn
     + '&allColumnType=' + allColumnType
     + '&indexCount=' + tableIndexCount
     + '&indexNameArray=' + indexNameArray.join('@')
     + '&indexUniqueArray=' + indexUniqueArray.join('@')
     + '&indexKeyColumnArray=' + indexColumnArray.join('@')
     + '&indexIncludeColumnArray=' + indexIncludeArray.join('@')
     + '&indexTypeArray=' + indexTypeArray.join('@')
     + '&timstamp=' + timstamp, '', 'edge:raised;resizable:yes;status:no;dialogWidth=650px;dialogHeight=300px;center=yes;help=no');

    if (retVal != null) {
        var retArray = retVal.split('|');
        for (var i = 0; i < allIndex.length; ) {
            if (allIndex[i].tablename == tableName) {
                allIndex.splice(i, 1);
            }
            else
                i++;
        }
        //        parent.window.returnValue = indexNameArray.join(',') +
        //         "|" + indexTypeArray.join(',') +
        //          "|" + indexUniqueArray.join(',') +
        //           "|" + keyColumnsArray.join('@') +
        //            "|" + includeColumnsArray.join('@');

        for (var i = 0; i < retArray[0].split(',').length; i++) {
            var tempIndex = new IndexBase();
            tempIndex.tablename = tableName;
            if ($.trim(retArray[0].split(',')[i]) != "") {
                tempIndex.name = retArray[0].split(',')[i];
                tempIndex.type = retArray[1].split(',')[i];
                tempIndex.unique = retArray[2].split(',')[i];
                tempIndex.keyColumns = retArray[3].split('@')[i];
                tempIndex.IncludeColumns = retArray[4].split('@')[i];
                allIndex.push(tempIndex);
            }
        }

        for (var i = 0; i < this.selection[0].rows.length; i++) {
            OZ.DOM.removeClass(this.selection[0].rows[i].dom.title, "primary");
            //            this.selection[0].rows[i].data.ai = false;
            for (var j = 0; j < this.selection[0].rows[i].keys.length; j++) {
                var k = this.selection[0].rows[i].keys[j];
                k.type = "";
            }
        }
        var primaryColumn = '';
        for (var i = 0; i < allIndex.length; i++) {
            if (allIndex[i].tablename == tableName && allIndex[i].type == "PRIMARY") {
                primaryColumn = allIndex[i].keyColumns;
                break;
            }
        }
        ...
        for (var i = 0; i < this.selection[0].rows.length; i++) {
            var tempPrimaryColumn = ',' + primaryColumn + ','
            var currentColumn = ',' + this.selection[0].rows[i].data.title + ',';
            if (tempPrimaryColumn.lastIndexOf(currentColumn) > -1) {
                OZ.DOM.addClass(this.selection[0].rows[i].dom.title, "primary");
                for (var j = 0; j < this.selection[0].rows[i].keys.length; j++) {
                    var k = this.selection[0].rows[i].keys[j];
                    k.type = "PRIMARY";
                }
...
SQL.RowManager.prototype.init = function (owner) {
    ...
    var ids = ["foreignconnect", "foreigndisconnect", "removerow"]; //"editrow",,"uprow","downrow",
    for (var i = 0; i < ids.length; i++) {
        var id = ids[i];
        var elm = OZ.$(id);
        this.dom[id] = elm;
...
//by ????
//???????????
SQL.Row.prototype.DeleteR = function (e) {
    isdrag = false;
    $("#removerow").click();
}
...
SQL.RowManager.prototype.foreigndisconnect = function (e) { /* remove connector */
    ...
    for (var i = rels.length - 1; i >= 0; i--) {
        ...
        if (r.row1 == this.selected) { this.owner.removeRelation(r); }
    }
    this.redraw();
}
...
SQL.RowManager.prototype.remove = function (e) {
    ...
    //add by wyzhang
    var tbName = t.data.title.split('(')[0];
    var primaryIndexUseFlag = false;
    for (var i = 0; i < allIndex.length; ) {
        if (allIndex[i].tablename == tbName) {
            var keyColumns = allIndex[i].keyColumns;
            var includeColumns = allIndex[i].IncludeColumns;
            var tempkeyColumns = ',' + keyColumns + ',';
            var tempincludeColumns = ',' + includeColumns + ',';
            var curColumnName = ',' + this.selected.getTitle() + ',';
            if (tempkeyColumns.lastIndexOf(curColumnName) > -1 || tempincludeColumns.lastIndexOf(curColumnName) > -1) {
                primaryIndexUseFlag = true;
                break;
            }
        }
        i++;
    }
    if (primaryIndexUseFlag) {
        alert(_("PrimaryIndexColumnUsing"));
        delsuccess = false;
        return;
    }
...
//        this.dom.foreignconnect.disabled = !(this.selected.isUnique());
        ...
        for (var i = 0; i < rels.length; i++) {
            ...
            if (r.row1 == this.selected) { this.dom.foreigndisconnect.disabled = false; }
        }
...
SQL.KeyManager.prototype.build = function () {
    ...
    var types = ["PRIMARY", "NONCLUSTERED_INDEX", "NONCLUSTERED_INDEX_INCLUDE", " NONCLUSTERED_INDEX_UNIQUE"];
    //["PRIMARY","CLUSTERED_INDEX ","NONCLUSTERED_INDEX","NONCLUSTERED_INDEX_INCLUDE","UNIQUE"];//,"FULLTEXT"
    for (var i = 0; i < types.length; i++) {
        var o = OZ.DOM.elm("option");
        o.innerHTML = types[i];
        o.value = types[i];
...
SQL.KeyManager.prototype.add = function (e) {//???????????  ?????????????
    var type = (this.table.keys.length ? "NONCLUSTERED_INDEX" : "PRIMARY"); // (this.table.keys.length ? "INDEX" : "PRIMARY");
...
SQL.KeyManager.prototype.switchTo = function (index) { /* show Nth key */
    ...
    for (var i = 0; i < this.table.rows.length; i++) {
        ...
        if (k.rows.indexOf(r) != -1 || r.data.title == "NewColumn") { continue; }
        var o = OZ.DOM.elm("option");
...
var editTable = '';//add by wyzhang
SQL.Window.prototype.ok = function (e) {
    if ($("#tablecomment").val() != null && $.trim($("#tablecomment").val()) == "") {
        $("#tablecomment").css("border", "solid 2px #F00")
        return;
    }
    else
        $("#tablecomment").css("border", "2px solid ");

    var CurrentTableName = $.trim($("#tablename").val()).split('(')[0];    //add by wyzhang

    var hasNewTableCreate = false;
    for (var i = 0; i < this.owner.tables.length; i++) {
        if (this.owner.tables[i].data.title == "TableName") {
            hasNewTableCreate = true;
            editTable = "";
            break;
        }
    }
    ...
    if ($.trim(editTable) == "") {
        var isExist = false;
        for (var i = 0; i < allIndex.length; i++) {
            if (allIndex[i].tablename == CurrentTableName) {
                isExist = true;
            }
        }
        if (!isExist) {
            var tempIndex = new IndexBase();
            tempIndex.tablename = CurrentTableName;
            tempIndex.name = "PK_" + CurrentTableName;
            tempIndex.type = "PRIMARY";
            tempIndex.unique = "1";
            tempIndex.keyColumns = "NewColumn";
            tempIndex.IncludeColumns = "";
        ...
        if (hasNewTableCreate && CurrentTableName != "TableName") {//rename table

            for (var i = 0; i < allIndex.length; i++) {
                if (allIndex[i].tablename == "TableName") {
                    allIndex[i].tablename = CurrentTableName;
                }
            }
            editTable = "";
        }
        ...
        var count = 0;
        for (var i = 0; i < this.owner.tables.length; i++) {
            if (this.owner.tables[i].data.title.split('(')[0] == editTable) {
                count++;
            }
        }
        if (editTable != CurrentTableName) { //rename table
//            alert(editTable + '___' + CurrentTableName + '__' + count)
            for (var i = 0; i < allIndex.length; i++) {
                if (allIndex[i].tablename == editTable) {
                    if (count < 1) {
                        allIndex[i].tablename = CurrentTableName;
                        if (allIndex[i].type == 'PRIMARY') {
                            allIndex[i].name = "PK_" + CurrentTableName;
                        }
                    }
                    else {
                        var tempIndex = new IndexBase();
                        ...
                        if (allIndex[i].type == 'PRIMARY')
                            ...
                            tempIndex.name = allIndex[i].name;
                        tempIndex.type = allIndex[i].type;
                        tempIndex.unique = allIndex[i].unique;
                        tempIndex.keyColumns = allIndex[i].keyColumns;
                        tempIndex.IncludeColumns = allIndex[i].IncludeColumns;
                        allIndex.push(tempIndex)
                    }
                }
            }
            editTable = CurrentTableName;
        }
    }
    //this.owner.io.updatetextarea();
    this.close();
...
SQL.Window.prototype.close = function () {
    //     if($("#tablecomment").val()!=null && $.trim($("#tablecomment").val())=="")
    //       {
    //           $("#tablecomment").css("border","2px solid #FF0000")
    //          return ;
    //       }
    //            $("#tablecomment").css("border","")
...
SQL.Designer.prototype.getCookie = function () {
    ...
    for (var i = 0; i < parts.length; i++) {
        var part = parts[i];
        var r = part.match(/ctripdbdesigner=({.*?})/);
        if (r) { obj = eval("(" + r[1] + ")"); }
    }
    return obj;
}
...
SQL.Designer.prototype.setCookie = function (obj) {
    ...
    var str = "{" + arr.join(",") + "}";
    document.cookie = "ctripdbdesigner=" + str + "; path=/";
}
...
SQL.Designer.prototype.toSourceXML = function () {
